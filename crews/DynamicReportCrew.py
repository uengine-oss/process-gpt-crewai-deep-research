...
class DynamicReportCrew:
    ...
    def _build_task_description(self, base_description: str, context_info: str, user_id: str, tenant_id: str) -> str:
        ...
        **4단계: 섹션별 적합한 이미지 생성**
        - **image_gen 도구 활용**: 현재 섹션 '{self.section_title}'의 내용과 컨텍스트에 맞는 적절한 이미지 생성
        - **이미지 생성 원칙**: 
        ...
        **🎯 도구 활용 원칙:**
        ...
        - **이미지 생성 활용**: 각 섹션에는 반드시 image_gen 도구를 실제로 호출하여 전문적인 이미지 생성
        ...
        **📊 내용 구성 원칙:**
        ...
        - **단계별 도구 활용**: 1단계(피드백 검토) → 2단계(객관적 정보 수집) → 3단계(최신 정보 보완) → 4단계(이미지 생성) 순서로 진행
        ...
        **🖼️ 이미지 생성 및 삽입 규칙:**
        - **모든 섹션에 이미지 필수**: 각 섹션마다 반드시 image_gen 도구를 실제로 호출하여 적절한 이미지 생성
        - **실제 도구 호출만 허용**: image_gen 도구를 실제로 호출해야 함 (텍스트로만 작성 금지)
        - **도구 결과 그대로 삽입**: image_gen 도구 호출 결과를 섹션 내용 중 적절한 위치에 그대로 삽입
        - **성공한 경우만 삽입**: 도구 호출이 성공한 경우에만 결과를 삽입, 실패한 경우 아무것도 넣지 않음
    ...
    def _build_expected_output(self, expected_output: str) -> str:
        ...
        **📝 출력 형식:**
        ...
        - 이미지 삽입: image_gen 도구 결과를 그대로 삽입
