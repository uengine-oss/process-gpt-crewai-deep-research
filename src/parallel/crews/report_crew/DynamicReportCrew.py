...
class DynamicReportCrew:
    ...
    def create_section_task(self, agent: Agent) -> Task:
        ...
        🛠️ 도구 사용 필수 규칙:
        - mem0 도구 사용 시 반드시 query 파라미터에 구체적인 검색어 입력
        - perplexity 도구 사용 시 명확한 질문이나 키워드 제공
        - 도구 파라미터 query를 null이나 빈 값으로 전달하지 말 것
        - 예시: mem0(agent_name="reporter", mode="retrieve", query="해외여행 안전수칙")
