...
class CrewAIEventLogger:
    ...
    def emit_feedback_started_event(self, feedback_json: dict, todo_id: str = None, proc_inst_id: str = None, job_id: str = "feedback"):
        """
        피드백 생성 시작 이벤트 기록
        ...
        self._write_to_backends(event_record)
        print(f"📝 [feedback_started] [feedback] {job_id[:8]} → Supabase: {'✅' if self.supabase_client else '❌'}")

    def emit_feedback_completed_event(self, feedback_json: dict, todo_id: str = None, proc_inst_id: str = None, job_id: str = "feedback"):
        """
        피드백 생성 완료 이벤트 기록
        feedback_json: {agent, role, feedback} 등
        todo_id: TODO 리스트 레코드 ID
        ...
        self._write_to_backends(event_record)
        print(f"📝 [feedback_completed] [feedback] {job_id[:8]} → Supabase: {'✅' if self.supabase_client else '❌'}")
    ...
    def emit_feedback_event(self, event_type: str, feedback_json: dict, todo_id: str = None, proc_inst_id: str = None, job_id: str = "feedback"):
        """
        피드백 생성 전후 이벤트 기록 (레거시 호환성용 - 사용 권장하지 않음)
        event_type: 'feedback_started' 또는 'feedback_completed'
        feedback_json: {agent, role, feedback} 등
        todo_id: TODO 리스트 레코드 ID
        proc_inst_id: 프로세스 인스턴스 ID
        """
        if event_type == "feedback_started":
            self.emit_feedback_started_event(feedback_json, todo_id, proc_inst_id, job_id)
        elif event_type == "feedback_completed":
            self.emit_feedback_completed_event(feedback_json, todo_id, proc_inst_id, job_id)
        else:
            # 기본적으로 feedback_completed로 처리
            self.emit_feedback_completed_event(feedback_json, todo_id, proc_inst_id, job_id)
